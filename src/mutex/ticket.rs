...
impl<T: ?Sized, R: RelaxStrategy> TicketMutex<T, R> {
    ...
    /// Locks a pinned [`TicketMutex`] and returns a pinned version of the mutex guard.
    /// 
    ...
    /// let lock = spin::mutex::TicketMutex::<_>::new(my_future);
    ...
    /// ```
    #[inline(always)]
    pub fn lock_pinned(self: Pin<&Self>) -> Pin<TicketMutexGuard<T>> {
...
impl<T: ?Sized, R> TicketMutex<T, R> {
    ...
    /// Tries to lock a pinned [`TickeMutex`] and returns a pinned version of the mutex guard.
    /// 
    /// This is useful for when the data is pinned in memory, such as when it is stored in a
    /// future.
    /// 
    ...
    /// ```
    #[inline(always)]
    pub fn try_lock_pinned(self: Pin<&Self>) -> Option<Pin<TicketMutexGuard<T>>> {
